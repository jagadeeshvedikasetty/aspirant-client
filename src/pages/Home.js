...
import { getSubjects, getTopics, getDates, getQuestions, getTests, generateTest, getMarks } from '../api';
...
function Home() {
  ...
  const handleStartTest = async (testId) => {
    setError('');
    try {
      // Load marks from server (primary source)
      const marksRes = await getMarks(testId);
      const serverMarks = marksRes.data;
      const has1x = serverMarks['1x'] && serverMarks['1x'].length > 0;
      const has2x = serverMarks['2x'] && serverMarks['2x'].length > 0;
      if (has1x || has2x) {
        const test = tests.find((t) => t._id === testId);
        setModalTestId(testId);
        setModalMarks(serverMarks);
